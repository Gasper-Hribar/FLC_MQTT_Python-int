...
def collect_range_data(file):
    range_data = pd.read_excel(file, sheet_name='Sheet1', usecols='AR:AX')
    range_data.fillna(value=0)
    names = range_data.iloc[[0]]
    values = range_data.iloc[[1]]
    names1 = [str(i) for i in names.iloc[0]]
    values1 = [float(i) for i in values.iloc[0]]
    dac_dict = {names1[i]: values1[i] for i in range(0, 3)}
    adc_dict = {names1[i]: values1[i] for i in range(3, len(values1))}
...
class FLC_interface:
    def __init__(self, serial_settings:SerialSettings, settings:list[PortSetting], initVals:list[PortInit]):
        ...
        self.arduino.close()
        self.arduino.open()
        self.initialize_ADDrange()
        self.initialize_ports()

    def initialize_ADDrange(self):
        # dac_range, adc_range = collect_range_data()
