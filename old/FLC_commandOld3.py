...
class PortSetting:
    ...
    def updateChipSettings(self):
        ...
        self.add1_settings = self.collect_chip_data(add1, 'add')
        self.add3_settings = self.collect_chip_data(add3, 'add')
        self.add4_settings = self.collect_chip_data(add4, 'add')
        self.mcp_settings = self.collect_chip_data(mcp, 'mcp')


    def collect_chip_data(self, chip, name):
        slovarcek = {'ADC':1, 'DAC':2, 'DIG_IN':3, 'DIG_OUT':4}
        slovarMCP = {'DIG_OUT': 0, 'DIG_IN': 1}
        completeChip = []
        chipFunction = []
        ...
        for r in range(len(chip)):
            if chip[r] is None:
                completeChip.append(ChSetting(function='DIG_IN', name=f'hidden_channel{r}', unit='0', k=0.0, constant=0.0, hidden=True, coordX=0, coordY=0, dacRange=0, adcRange=0, minValues=0.0, maxValues=0.0, initValues=0.0, isImportant=False, Ximportant=0, Yimportant=0, activeLow=False, isExp=False, B=0.0, r0=0.0, t0=0.0, enableInitVal=False, disablePowerDrop=False))
            else:
            ...
            if name == 'mcp':
                for config in slovarMCP:
                    if channel.function == config:
                        chipFunction.append(slovarMCP[config])
                    else:
                        pass
            elif name == 'add':
                for config in slovarcek:
